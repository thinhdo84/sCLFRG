...
classdef Lyapunov_NNsigmoid< handle
    ...
    methods (Access = public)
        function obj = Lyapunov_NNsigmoid(Weight, bias)
            nx = size(Weight{1},2);
        ...
        function dV = grad(obj,X)
            ...
            for k = 1:obj.nx
                for i =1:obj.L
                    dV(k) = dV(k)...
              +obj.Weight{2}(i)*dsigmoid(obj.Weight{1}(i,:)*X+obj.bias{1}(i))*obj.Weight{1}(i,k);
                end
            end
        end
    end
...
y = zeros(n,1);
for i = 1:n
    y(i) = sigmoidActive(x(i));
end
end

function dy = dsigmoid(x)
dy = sigmoidActive(x) * (1-sigmoidActive(x));
end
